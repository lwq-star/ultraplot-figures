...
- 将内部合同、QA helper、临时检查文件和正式交付物分开，使交付脚本保持独立，并在作者复核前核对成图是否符合原始要求。
...
- [预测值与真实值模型对照案例](examples/correlation-scatter-plot/README_zh.md)：
  使用相同 Excel 数据和相同提示词，对比 LR、SVR、GBRT 和 DNN 在四类土地
  类型下的图件。案例包含输入数据、可编辑脚本、PDF 和 TIFF、预览图及文件信息。
...
内部任务合同和 Skill QA 调用只保留在临时核验代码中，不会写入交付的绘图脚本。
...
4. 交付的 Python 绘图脚本是否能够独立运行，且不包含内部合同、证据注册表或 Skill QA 调用；
5. 按实际使用尺寸查看时，图件是否容易理解。
